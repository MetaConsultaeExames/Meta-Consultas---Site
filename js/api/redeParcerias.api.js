...
function getMedicsByHour() {
    $.post(API + "/agendamento",
    {
        ...
        "inicio": DATAPROCEDIMENTO.inicio,
        "termino": DATAPROCEDIMENTO.termino,
    ...
    function(data, status) {
        if(data.data != null && data.data.length > 0) {
            ...
            $.each(data.data, function(index, object) {
                ...
                valueDesconto = String(valueDesconto).slice(0, -2) + ',' + String(valueDesconto).slice(-2);
...
function buscarTodosPacientesConsultasDiaAnterior() {
    $.ajax({
        url: API + "/redeparcerias/?allPacientes=consultas",
        method: "get",
        success: (obj) => {
            if(obj.data != null) {
                
                const content = processAgendamentos(obj.data.content);
                window.processedContent = content;
        ...
        complete: () => {
            if (window.processedContent) {
                downloadFile(window.processedContent, 'redeParcerias.txt');
            }
        }
...
// Função para calcular data_encerramento (data + 60 dias)
...
// Função para formatar a linha conforme o padrão solicitado
function formatLine(agendamento) {
    const dataEncerramento = calculateEndDate(agendamento.data);
    return `[ agendamento_id:${agendamento.agendamento_id} | paciente_id:${agendamento.paciente_id} | data:${agendamento.data} | horario:${agendamento.horario} | data_encerramento:${dataEncerramento} ]`;
}
...
// Função para processar uma lista de agendamentos e retornar o conteúdo formatado
function processAgendamentos(agendamentos, existingContent = '') {
    // Formata todas as linhas dos agendamentos
    const newLines = agendamentos.map(formatLine);
    
    // Se houver conteúdo existente, processa para atualizar ou adicionar linhas
    const lines = existingContent ? existingContent.split('\n').filter(line => line.trim() !== '') : [];
    const updatedLines = [...lines];

    newLines.forEach(newLine => {
        const agendamentoId = newLine.match(/agendamento_id:(\d+)/)[1];
        const index = updatedLines.findIndex(line => line.includes(`agendamento_id:${agendamentoId}`));
    ...
    // Retorna o conteúdo final como uma string
        ...
        success: (obj) => {
            if(obj.data != null ) {
                ...
                selectList.onchange = function() {
                    ...
                    getProcedimento(selectedId, selectedText, selectedValor, obj);
                };
