    ...
    function(data, status) {
        if(data.data != null && data.data.length > 0) {
            ...
            $.each(data.data, function(index, object) {
                ...
                // Iterar sobre cada local_id
                $.each(object.data, function(localIndex, localData) {
                    let orderedDates = Object.keys(localData.horarios).sort();
                    $.each(orderedDates, function(i, date) {
                        let dateValue = localData.horarios[date];
                        ...
                        if(dateValue.length > 0) {
                            ...
                            // ID único do botão
                            const diaId = `dia_${object.profissional_id}_${localData.local_id}_${i}`;

                            diasDisponiveis += `
                                <li class='list-group-item list-dia' id="${diaId}" 
                                    style="cursor:pointer; color: #46bee6;font-weight: 600;" 
                                    onclick="showHourModal('${object.profissional_id}', '${object.profissional_nome}', '${date}', '${JSON.stringify(dateValue).replace(/"/g, '&quot;')}', '${localData.local_id}')">
                                    ${labelDia}
                                </li>
                            `;
                        }
            ...
            alert("Informamos que infelizmente, não temos disponibilidade desta especialidade nos próximos 20 dias na unidade escolhida. Porém você pode optar por agendar em outra unidade próxima!")
            location.reload();
        }
    });
}

function showHourModal(profissional_id, profissional_nome, date, horariosJson, local_id) {
    ...
    horarios.forEach((hour, i) => {
        ...
        item.innerHTML = `
            <li class='list_hour' 
                ...
                onclick='getProfessionalDateHour(${i}, ${profissional_id}, "${profissional_nome}", "${date}", "${hour}", "${local_id}")'>
                ${hora}
            </li>
            <hr>
        `;
        ...
        success: (obj) => {
            if(obj.data != null) {
                ...
                $.each(obj.data.content, function(i, d) {
                    $.each(d, function(x, y){
                        if(y.ExibirAgendamentoOnline == 1) {
...
async function buscarClientes(datanasc, cpf, clear) {
    ...
    //await buscaDadosReceita(cpftratado)
...
async function appointmentSubmit() {
    ...
    $.ajax({
        url: API + "/agendamento/?newAppoint=create&localId="+ AGENDAMENTO.local_id + "&profissionalId="+ AGENDAMENTO.profissional_id + "&valor=" + AGENDAMENTO.valor +
            ...
            "&data="+ data +"&hora="+ AGENDAMENTO.horario +"&celular0="+ cell +"&email="+ AGENDAMENTO.email,
        method: "get",
        success: (obj) => {
            if(obj.data != null ) {
